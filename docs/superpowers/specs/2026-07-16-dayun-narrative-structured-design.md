...
| 状态 | 待用户审阅规格 |
| 日期 | 2026-07-16 |
| 决策 | 用户同意方案 B：数据与讲解分流；API 结构化 + 新 UI |
| 范围 | 八字深读「大运叙事」块（`NewBaziView` / dayun-report）；报告卷三同源若共用 schema 一并兼容 |
...
- 河洛易数、奇门遁甲、盲派独立推算或假接口。
- 改写十神模板文案策略 / 字数 KPI（400–600 字）本身（结构拆出后总量可不变）。
- 自动 LLM 大运（仍保持按需加载）。
