...
### 权限层面
- [ ] 所有角色在constants.py中定义
- [ ] 所有权限检查使用统一的has_permission()方法
- [ ] 所有API端点都标注@require_permission装饰器
- [ ] GUEST/SHARED_LINK的过期时间由TTL强制执行
- [ ] 没有"超级权限"后门（即使ADMIN也需经过权限check）

### 审计层面
- [ ] 所有数据变更操作都产生审计日志
- [ ] 审计日志存储在单独的表/库中，不能被业务逻辑删除
- [ ] 敏感字段在日志中按rules脱敏，但hash不变（用于检测篡改）
- [ ] 每条审计日志都有链式签名，防止事后篡改
- [ ] 定期（每周）验证审计链完整性，异常报警
- [ ] 审计日志导出功能仅ADMIN/AUDITOR可用
- [ ] 审计日志保留期>=3年

### 前端显示
- [ ] 根据用户角色动态隐藏菜单项（不是安全边界！）
- [ ] 禁用操作时清晰提示原因（"您无权修改他人数据"）
- [ ] 显示操作者身份（OWNER vs DELEGATE vs FAMILY_MEMBER）
- [ ] 脱敏数据清晰标记（"姓名已脱敏"）
