...
def get_current_user_from_token(request: Request, session: Session = Depends(get_session)) -> TokenPayload:
    """
...
# ✅ 别名，用于依赖注入
async def require_user(user: TokenPayload = Depends(get_current_user_from_token)) -> TokenPayload:
    """强制认证的依赖函数"""
...
@router.get("/auth/me")
@handle_exceptions(ErrorCode.SYSTEM_INTERNAL_ERROR)
def get_current_user_info(session: Session = Depends(get_session), user: TokenPayload = Depends(require_user)):
    """
    ...
    if len(parts) == 2 and parts[0].lower() == "bearer":
        try:
            ...
            if jti:
                ...
                expires_at = datetime.fromtimestamp(exp_ts, tz=UTC) if exp_ts else None
...
@handle_exceptions(ErrorCode.SYSTEM_INTERNAL_ERROR)
def change_password(
    body: ChangePasswordRequest, session: Session = Depends(get_session), user: TokenPayload = Depends(require_user)
):
    """
