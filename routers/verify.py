...
from app.pkg_utils import backend_status as _backend_status
...
from services.bazi_engine.dayun import _ELEM_CHILD_HINT as _CHILD_HINTS
from services.bazi_engine.dayun import _ELEM_LOVE_HINT as _LOVE_HINTS  # 红线5
from services.bazi_engine.relations import get_branch_relations, get_stem_clashes  # 红线14 / P0-11
...
def _build_legacy_verify_response(body, dt, lon, req_id, warnings):
    ...
    try:
        ...
        logger.exception(
            "Unexpected error in verify", extra={"request_id": req_id, "error_type": type(exc).__name__}, exc_info=True
        )
    ...
    _neutral_penalty = sum(
        max(0.0, _wx_dict.get(e, 0.0) - 25)
        for e in ["wood", "fire", "earth", "metal", "water"]
        if e not in _favor and e not in (yongshen.avoid or [])
    )
    ...
    social = SocialModel(
        ...
        social_hint=f"用神:{'/'.join(yongshen.favor)} 忌神:{'/'.join(yongshen.avoid)}"
    ...
    try:  # P0-11
        verify_response.tiangan_clashes = get_stem_clashes(rp.year.stem, rp.month.stem, rp.day.stem, rp.hour.stem)
...
def api_verify(
    ...
    if _bazi_engine_service._engine_v2_enabled():
        try:
            ...
            logger.warning("Verify validation error [v2]", extra={"request_id": req_id, "error": str(exc)})
            raise HTTPException(status_code=400, detail="Invalid input parameters")
        except Exception as exc:
            record_verify_metrics(
                mode=body.mode, boundary_level="", duration_secs=time.time() - _verify_start, success=False
        ...
        record_verify_metrics(
            mode=body.mode, boundary_level=_bl_v2, duration_secs=time.time() - _verify_start, success=True
        )
    ...
    try:
        ...
        raise
    record_verify_metrics(
        mode=body.mode, boundary_level=_bl_legacy, duration_secs=time.time() - _verify_start, success=True
    )
    return UnescapedJSONResponse(
