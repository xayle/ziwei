...
def get_config() -> LlmConfigResponse:
    ...
    return LlmConfigResponse(
        ...
        model=cfg.model,
        available=True,
        note="当前为 Mock 模式（无真实 API Key），生成模板化草稿" if is_mock else f"已配置 {cfg.provider.value} / {cfg.model}",
    )
...
async def interpret(
    ...
    if keywords:
        ...
        evidence = [
            f"《{e['title']}》：{e['passage']}" for e in raw_evidence
        ]
...
async def stream(
    ...
    async def event_gen():
        full_text_buf = []
        usage_buf = {}
        async for sse_msg in stream_interpretation(
...
def list_drafts(
    ...
    chart_hash: Optional[str] = Query(None, max_length=64),
...
class BaziInterpretRequest(_PydanticBase):
    ...
    case_id: str
    module: Optional[str] = None          # 可选：聚焦某模块（career_detail 等）
    chart_hash: Optional[str] = None      # 可选：客户端自定义 hash，默认按 case_id 派生
...
async def interpret_bazi(
    ...
    # 2. 确定 chart_hash（幂等键）
    chart_hash = payload.chart_hash or _hashlib.sha256(
        f"bazi:{payload.case_id}:{payload.module or ''}".encode()
    ).hexdigest()[:24]
    ...
    if _cached is not None:
        ...
        logger.debug("W4 cache hit: rendered_facts for case=%s module=%s",
                     payload.case_id, payload.module)
    else:
        try:
    ...
    evidence_strs: list[str] = [
        f"《{e.get('title', '')}》：{e.get('passage', e.get('excerpt', ''))}"
