...
import { fetchZiweiExplainBatchWithMeta, type ExplainBatchResponse } from '@/api/explain'
import { ZIWEI_PAGE_EXPLAIN_SECTIONS } from '@/constants/feBeContract'
import type { ProfileData } from '@/stores/profile'
import { buildZiweiRequest } from '@/utils/buildChartRequests'
import { explainSectionsToAnalysisBlocks } from '@/utils/explainSectionsToAnalysisBlocks'

export function useZiweiPageExplain() {
  ...
  async function loadPageExplain(profileData: ProfileData) {
    ...
    const result = await fetchZiweiExplainBatchWithMeta(
