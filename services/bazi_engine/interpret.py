...
_GENERAL_TMPL = {
    "balanced": (
        "命局五行较为均衡，整体运势平稳，既无大起大落之象，也无短板之忧。"
        "宜顺势而为，各方面稳步推进，不必刻意强化某一五行。"
    ),
    "weak_day_master": (
        "日主偏弱，需重视自身积累与资源保护，在行旺运时积极出击，"
        "在行衰运时学会守成，不要轻易消耗精力于无益之事。"
    ),
    "strong_day_master": (
        "日主偏旺，气场强劲，有充分的行动力，但需防过刚则折，"
...
_YONGSHEN_TMPL = (
    "命局用神为{favor_cn}，忌神为{avoid_cn}。"
    "凡行{favor_cn}大运或逢{favor_cn}流年，事业、财运、健康均有正向推动。"
    "逢{avoid_cn}运则需低调守成，避免重大决策。"
...
def interpret_bazi(inp: InterpretInput) -> InterpretResult:
    ...
    # ── 3. 格局解读 ──────────────────────────────────────────────────────────
    result.geju_text = _GEJU_TMPL.get(
        inp.geju_name,
        _GEJU_DEFAULT_TMPL.format(geju_name=inp.geju_name)
    )
