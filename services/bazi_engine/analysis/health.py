...
# ── 日主天干专项健康关注点（10天干）────────────────────────────────────────────
_STEM_HEALTH_FOCUS: dict[str, str] = {
    "甲": "甲木日主进取心旺，最大健康隐患是长期高压导致的肝气郁结与颈肩肌肉群紧张；建议每周安排一次深度放松（泡浴/按摩/冥想），主动释放累积的精神张力。",
    "乙": "乙木日主情感细腻，长期压抑情绪易引发肝血不足与眼睛疲劳；规律使用护眼措施，并学会通过倾诉或创作出口疏导内在积郁，防止情绪性免疫低下。",
    "丙": "丙火日主热情充沛，心脑血管是长期关注重点；避免过度兴奋后的急剧情绪跌落，夏日防暑降温不可疏忽，限制咖啡因摄入以稳定自主神经节律。",
    "丁": "丁火日主细腻敏感，失眠与神经性偏头痛是高频困扰；睡前1小时进行数字设备戒断，养成写感恩日记的习惯，以正向思维替代过度反刍思考。",
    "戊": "戊土日主厚重稳重，脾胃湿气偏重是主要体质弱点；饭后散步10分钟助消化，减少甜腻食物与冷饮，保持规律代谢节律防止体重过度增加。",
    "己": "己土日主细腻内敛，消化系统与血糖平稳是核心健康议题；规律三餐时间、避免情绪化暴食，轻柔运动（散步/八段锦）比剧烈运动更适合己土体质。",
    "庚": "庚金日主刚毅，肺与大肠功能需长期维护；规律腹式呼吸与户外有氧，秋季重点防呼吸道感染，情绪压制习惯可能引发便秘，适当排解压力为首要。",
    "辛": "辛金日主精致敏锐，皮肤与呼吸系统最为敏感；保持室内湿度45-60%、使用温和护肤产品，防止秋燥与化学过敏，适当补充维生素C与锌强化免疫屏障。",
    "壬": "壬水日主灵动好动，肾脏与泌尿系统是长期关注焦点；避免熬夜耗损肾精，保持充足饮水（日均1500ml以上），冬季重点防寒保暖腰腹与足底。",
    "癸": "癸水日主深思内省，肾虚与关节寒湿是潜在体质弱点；避免久坐不动与长期精神过载，冬季配合补肾食材（枸杞/黑芝麻/核桃），定期做腰背肌肉强化练习。",
}
...
def compute_health(
    ...
    # ─── 日主天干专项健康关注 ────────────────────────────────────────────
    stem_health_focus = _STEM_HEALTH_FOCUS.get(day_stem, "") if day_stem else ""
    ...
    return HealthAnalysisModel(
        ...
        interpretation_text=(
            f"健康评分为 {health_score} 分，整体风险等级【{risk_level}】。"
            f"{_risk_str}\n"
            + (f"【日主健康特质】{stem_health_focus}\n" if stem_health_focus else "")
