...
def compute_career(
    ...
    if leadership:
        development_advice = (
            f"官杀有力且日主中和，具备较高才测的领导潜质，建议主动争取管理职位或项目负责人。"
            f"利用{geju_name}格局特点，在公司或行业内辅展多方人脉，建立跳跃式资源网络。"
            f"界定未来 3-5 年精进目标，系统性提升管理方法论与岗位可見度。"
        )
    elif shi_shang_pct >= 0.4:
        development_advice = (
            f"食伤旺盛，创造力与表达力尤为突出，最适合技术/创意/自由职业路线，谨慎进入官僚体系。"
            f"建议持续输出高质量作品或方案，将个人 IP 与专业能力振定、变现。"
            f"可考虑展示个人展作集、申请行业各类奖项或将创意成果产品化、商业化。"
        )
    else:
        development_advice = (
            f"该命局适合稳步深耕，系统建训{geju_name}相关领域的专业技能体系。"
            f"天干地支大运顺进年决策，平时塬实基础、考证升级及建立跨部门协作能力。"
