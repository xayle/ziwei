...
def self_check() -> None:
    ...
    # 表3: 天干五行10条
    assert len(STEM_ELEMENT) == 10, "STEM_ELEMENT should have 10 entries"

    # 表4: 地支五行12条
    assert len(BRANCH_ELEMENT) == 12, "BRANCH_ELEMENT should have 12 entries"

    # 表5: 藏干权重和 = 1.0
    for branch, hidden in BRANCH_HIDDEN_STEMS.items():
        total = sum(w for _, w in hidden)
        assert abs(total - 1.0) < 1e-9, f"BRANCH_HIDDEN_STEMS[{branch}] weights sum to {total}, expected 1.0"
    assert len(BRANCH_HIDDEN_STEMS) == 12, "BRANCH_HIDDEN_STEMS should have 12 entries"

    # 表6: 十神表应含所有组合
    assert len(TEN_GOD_TABLE) == 100, f"TEN_GOD_TABLE should have 100 entries (10x10), got {len(TEN_GOD_TABLE)}"

    # 表7: 月令旺衰12支
    assert len(WANGXIANG) == 12, "WANGXIANG should have 12 entries"
    for branch, vals in WANGXIANG.items():
        assert set(vals.keys()) == {"wood", "fire", "earth", "metal", "water"}, f"WANGXIANG[{branch}] missing elements"

    # 表8: 六冲12条（6对）
    assert len(BRANCH_CHONG) == 12, "BRANCH_CHONG should have 12 entries"
    for a, b in BRANCH_CHONG.items():
        assert BRANCH_CHONG[b] == a, f"BRANCH_CHONG not symmetric for {a}"

    # 表9: 三合4局
    assert len(SAN_HE) == 4, "SAN_HE should have 4 entries"
