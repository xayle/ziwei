...
def _assess_zone(
    ...
    if rules.get("skip"):
        return ZoneAssessment(
            direction=direction, direction_zh=direction_zh,
            label=label, level_css=level_css,
            room_type=room_type, room_zh=room_zh,
            assess_level="skip", assess_score=0, assess_note="",
        )
    ...
    if rules.get("neutralizes"):
        ...
        return ZoneAssessment(
            ...
            label=label, level_css=level_css,
            room_type=room_type, room_zh=room_zh,
            assess_level=level, assess_score=score, assess_note=note,
        )
...
def assess_room_layout(
    ...
    for c in sorted(bad_cells, key=lambda x: x.assess_score):
        ...
        best_label_for_room = next(
            iter(_ROOM_RULES.get(c.room_type, {}).get("best_labels", set()) or {"生气"}), "生气"
        )
        best_dir = next(
            (d for item in bagua.auspicious if item.label == best_label_for_room for d in [item.direction]
