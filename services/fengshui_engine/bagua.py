...
class BaguaResult:
    ...
    # 家具建议
    bed_tip: FurnitureTip | None = None
    desk_tip: FurnitureTip | None = None
    door_tip: FurnitureTip | None = None

    # 房屋信息（可选）
    house_facing: str | None = None
    house_gua: int | None = None
    house_gua_name: str | None = None
    house_group: str | None = None
    compatibility: str | None = None  # 相合 / 不合
...
def calc_bagua(
    ...
    for lab in ji_labels:
        ...
        auspicious.append(
            DirectionItem(
                direction=d,
                direction_zh=DIRECTIONS_ZH[d],
                label=lab,
                level=info["level"],
                level_css=info["level_css"],
                desc=info["desc"],
            )
        )
