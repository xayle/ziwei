...
def _get_current_dayun(chart: ZiweiChart, year: int) -> DayunItem | None:
...
def _sihua_to_palace_map(
    ...
    for star, hua_text in sihua.items():
        for p in chart.palaces:
            if any(s["name"] == star for s in p.main_stars) or star in p.aux_names:
...
def _detect_events(
    ...
    if life_palace_name == "夫妻宫":
        pts += 4
        srcs.append(f"{period_label}命宫走入本命夫妻宫")
    elif life_palace_name == "福德宫":
        pts += 2
        srcs.append(f"{period_label}命宫走入本命福德宫")
    ...
    for pname in ("命宫", "夫妻宫", "福德宫"):
        ...
        if hit:
            pts += 1
            srcs.append(f"本命{pname}有{'、'.join(sorted(hit))}")

    # 化禄/化科飞入夫妻宫
    for hua_type in ("化禄", "化科"):
        if hua_pal.get(hua_type) == "夫妻宫":
            pts += 3
            srcs.append(f"{period_label}{hua_type}入夫妻宫")
        if dy_pal.get(hua_type) == "夫妻宫":
            pts += 2
            srcs.append(f"大运{hua_type}入夫妻宫")

    # 化忌入夫妻宫 → 减分（感情波折）
    if hua_pal.get("化忌") == "夫妻宫":
        pts -= 3
        srcs.append(f"{period_label}化忌入夫妻宫，感情有阻")

    if pts >= 6:
        events.append(
            EventTag(
                "桃花/姻缘", "强", "感情机遇极佳，桃花旺盛，单身者有缘分出现，已婚者感情升温", "；".join(srcs) or "无"
            )
        )
        score_delta += 10
    elif pts >= 3:
        events.append(
            EventTag("桃花/姻缘", "中", "感情有一定发展机遇，人际互动活跃，宜自然把握", "；".join(srcs) or "无")
        )
        score_delta += 5
    elif pts >= 1:
        events.append(
            EventTag("桃花/姻缘", "弱", "感情上有小动向，异性缘略有增加，留意身边人际变化", "；".join(srcs) or "无")
        )
        score_delta += 2
    elif pts <= -2:
        events.append(
            EventTag(
                "感情波折", "中", "感情关系有摩擦或阻碍，已婚者宜耐心沟通，暂缓重大感情决定", "；".join(srcs) or "无"
            )
        )
        score_delta -= 5
    ...
    if life_palace_name == "疾厄宫":
        pts += 3
        srcs.append(f"{period_label}命宫走入本命疾厄宫，健康为重点")

    # 化忌落健康宫位
    for pname in _HEALTH_PALACES:
        if hua_pal.get("化忌") == pname:
            pts += 5
            srcs.append(f"{period_label}化忌入{pname}")
        if dy_pal.get("化忌") == pname:
            pts += 3
            srcs.append(f"大运化忌入{pname}")

    # 双忌叠加（大运+流年化忌同落一宫）
    if hua_pal.get("化忌") and dy_pal.get("化忌") == hua_pal.get("化忌"):
        pts += 4
    ...
    for pname in _HEALTH_PALACES:
        ...
        if sha_here:
            pts += 1
            srcs.append(f"本命{pname}煞星{'、'.join(sorted(sha_here))}")
    ...
    if pts >= 8:
        ...
        events.append(
            EventTag(
                "灾祸/健康", "中", "健康或安全有一定隐患，注意劳逸结合，定期检查，避免冒险", "；".join(srcs) or "无"
            )
        )
        score_delta -= 10
    elif pts >= 2:
        events.append(
            EventTag("灾祸/健康", "弱", "有轻微健康波动或小风险，注意作息规律，适当养生", "；".join(srcs) or "无")
        )
        score_delta -= 4
    ...
    if hua_pal.get("化禄") in _WEALTH_PALACES:
        pts += 5
        srcs.append(f"{period_label}化禄入{hua_pal['化禄']}")
    if dy_pal.get("化禄") in _WEALTH_PALACES:
        pts += 3
        srcs.append(f"大运化禄入{dy_pal['化禄']}")
    if hua_pal.get("化权") == "财帛宫":
        pts += 3
        srcs.append(f"{period_label}化权入财帛宫")
    if life_palace_name in _WEALTH_PALACES:
        pts += 2
        srcs.append(f"{period_label}命宫走入本命{life_palace_name}")

    # 化忌入财帛宫/田宅宫 → 破财
    if hua_pal.get("化忌") in {"财帛宫", "田宅宫"}:
        pts -= 4
        srcs.append(f"{period_label}化忌入{hua_pal['化忌']}，防破财损耗")
    if dy_pal.get("化忌") == "财帛宫":
        pts -= 3
        srcs.append("大运化忌入财帛宫")

    if pts >= 6:
        events.append(
            EventTag("财运", "强", "财运旺盛，进财机会大，适合主动拓展财源，可进行稳健投资", "；".join(srcs) or "无")
        )
        score_delta += 12
    elif pts >= 3:
        events.append(EventTag("财运", "中", "财运平稳偏好，有小利可期，稳健理财为宜", "；".join(srcs) or "无"))
        score_delta += 6
    elif pts <= -4:
        events.append(
            EventTag("财运波折", "强", "财运有明显阻碍，破财风险高，暂缓大额投资，以守为主", "；".join(srcs) or "无")
        )
        score_delta -= 10
    elif pts < 0:
        events.append(EventTag("财运波折", "弱", "财运有小波折，注意日常开销与支出管控", "；".join(srcs) or "无"))
        score_delta -= 4
    ...
    if life_palace_name == "官禄宫":
        pts += 4
        srcs.append(f"{period_label}命宫走入本命官禄宫")
    if hua_pal.get("化权") in _CAREER_PALACES:
        pts += 4
        srcs.append(f"{period_label}化权入{hua_pal['化权']}")
    if dy_pal.get("化权") in _CAREER_PALACES:
        pts += 3
        srcs.append(f"大运化权入{dy_pal['化权']}")
    if hua_pal.get("化禄") == "官禄宫":
        pts += 3
        srcs.append(f"{period_label}化禄入官禄宫")
    if hua_pal.get("化科") in _CAREER_PALACES:
        pts += 2
        srcs.append(f"{period_label}化科入{hua_pal['化科']}")

    # 化忌入官禄/迁移 → 事业阻
    if hua_pal.get("化忌") in _CAREER_PALACES:
        pts -= 3
        srcs.append(f"{period_label}化忌入{hua_pal['化忌']}，事业有阻")
    if dy_pal.get("化忌") == "官禄宫":
        pts -= 2
        srcs.append("大运化忌入官禄宫")

    if pts >= 8:
        events.append(
            EventTag("事业/官运", "强", "事业运极佳，有升迁创业重大机遇，宜主动争取、积极表现", "；".join(srcs) or "无")
        )
        score_delta += 14
    elif pts >= 4:
        events.append(
            EventTag("事业/官运", "中", "事业稳步发展，有晋升或拓展机遇，积极表现可获认可", "；".join(srcs) or "无")
        )
        score_delta += 7
    elif pts <= -3:
        events.append(EventTag("事业挫折", "中", "事业有阻滞或口舌是非，宜低调行事，韬光养晦", "；".join(srcs) or "无"))
        score_delta -= 8
    elif pts < 0:
        events.append(EventTag("事业挫折", "弱", "事业有小阻力，注意职场人际，避免正面冲突", "；".join(srcs) or "无"))
        score_delta -= 3
    ...
    if gui:
        pts += 2
        srcs.append(f"本命命宫有{'、'.join(sorted(gui))}")
    ...
    if dy_pal.get("化科") in {"命宫", "官禄宫"}:
        pts += 2
        srcs.append(f"大运化科入{dy_pal['化科']}")
    if hua_pal.get("化禄") in {"交友宫", "命宫"}:
        pts += 2
        srcs.append(f"{period_label}化禄入{hua_pal['化禄']}，贵人助力")

    if pts >= 5:
        events.append(
            EventTag("贵人/助力", "强", "贵人运强，有重要贵人相助，人际关系顺畅，宜借势而为", "；".join(srcs) or "无")
        )
        score_delta += 8
    elif pts >= 2:
        events.append(EventTag("贵人/助力", "中", "有一定贵人助力，善用人脉可事半功倍", "；".join(srcs) or "无"))
        score_delta += 4
    ...
    if life_palace_name == "迁移宫":
        pts += 4
        srcs.append(f"{period_label}命宫走入本命迁移宫")

    for p in chart.palaces:
        if "天马" in p.aux_names and p.name in {"迁移宫", "命宫"}:
            pts += 2
            srcs.append(f"本命{p.name}有天马")

    for hua_type in ("化禄", "化权"):
        if hua_pal.get(hua_type) == "迁移宫":
            pts += 2
            srcs.append(f"{period_label}{hua_type}入迁移宫")

    if pts >= 4:
        events.append(
            EventTag("变动/迁移", "强", "变动性大，宜出行或移居，守旧难行，主动出击则柳暗花明", "；".join(srcs) or "无")
        )
        score_delta += 3
    elif pts >= 2:
        events.append(
            EventTag("变动/迁移", "中", "有一定变动倾向，可能涉及出行或工作地点变化，随缘应对", "；".join(srcs) or "无")
        )
        score_delta += 1
    ...
    for pname in ("交友宫", "迁移宫"):
        ...
        if sha:
            pts += 1
            srcs.append(f"本命{pname}有煞星{'、'.join(sorted(sha))}")

    if pts >= 4:
        events.append(EventTag("口舌/是非", "中", "人际关系易生是非，言行宜低调，防小人中伤", "；".join(srcs) or "无"))
        score_delta -= 4
    elif pts >= 2:
        events.append(EventTag("口舌/是非", "弱", "口舌小是非有机会出现，避免不必要争论", "；".join(srcs) or "无"))
        score_delta -= 2
...
def _build_details(
    ...
    if love_lu and dy_love_lu:
        details["感情"] = (
            f"{period_label}与大运双禄入夫妻宫，感情缘分强力叠加，感情婚恋时机极佳，单身者宜主动把握，伴侣感情大幅升温"
        )
    elif love_lu:
        details["感情"] = f"{period_label}化禄飞入夫妻宫，感情运佳，有缘分出现或感情深化，适合婚恋进展与表白"
    elif love_ke:
        details["感情"] = f"{period_label}化科飞入夫妻宫，感情名声有助，相处和谐，易得到认可与欣赏"
    elif love_ji:
        details["感情"] = f"{period_label}化忌飞入夫妻宫，感情有摩擦或阻碍，宜耐心沟通，暂缓重大感情决定，注意情感纠葛"
    elif life_palace_name == "夫妻宫":
        details["感情"] = f"{period_label}命宫走入夫妻宫，感情婚姻为本期主题，有明显起伏变化，宜用心经营"
    else:
    ...
    if wealth_lu_palace in _WEALTH_PALACES and dy_wealth_lu:
        ...
        details["财运"] = f"{period_label}化禄飞入{wealth_lu_palace}，财运活跃，进财机会增多，宜积极开源"
    elif wealth_ji:
        details["财运"] = f"{period_label}化忌入{hua_pal.get('化忌')}，财运有阻，需防破财损耗，暂缓大额投资，以守为主"
    elif life_palace_name in _WEALTH_PALACES:
        details["财运"] = f"{period_label}命宫走入{life_palace_name}，财运为本期重点，宜把握时机，开拓多元收入"
    else:
    ...
    if career_quan and dy_career:
        details["事业"] = (
            f"{period_label}化权与大运叠加入{hua_pal['化权']}，事业运最旺，升职或重大突破极可能，宜全力出击"
        )
    elif career_quan:
        details["事业"] = f"{period_label}化权入{hua_pal['化权']}，事业运强，有升职或拓展机会，积极争取可获认可"
    elif career_lu:
        details["事业"] = f"{period_label}化禄入官禄宫，事业顺遂，工作上有机会拓展，财官并见"
    elif career_ji:
        details["事业"] = (
            f"{period_label}化忌入{hua_pal['化忌']}，事业有一定阻滞，宜低调行事，韬光养晦，避免与上级正面冲突"
        )
    elif life_palace_name == "官禄宫":
        details["事业"] = f"{period_label}命宫走入官禄宫，事业为本期重心，宜积极把握升迁机遇"
    else:
        details["事业"] = "事业运势稳定，专注本职工作，循序渐进"

    # ── 健康 ────────────────────────────────────────────────
    health_ji = hua_pal.get("化忌") in _HEALTH_PALACES or dy_pal.get("化忌") in _HEALTH_PALACES
    if life_palace_name == "疾厄宫":
        details["健康"] = f"{period_label}命宫走入疾厄宫，健康为重心，宜积极养生保健，定期体检，避免过劳"
    elif hua_pal.get("化忌") == "命宫" and dy_pal.get("化忌") == "命宫":
        details["健康"] = "大运与流年双忌同落命宫，身心压力较大，务必重视健康管理，防大病"
    elif health_ji:
        details["健康"] = "化忌涉及健康宫位，注意身体状况，避免透支体力，有旧患者尤需留意"
    else:
...
def _build_advice(events: list[EventTag], palace_name: str) -> str:
    """根据事件列表及宫位主题生成综合行动建议。"""
    strong_bad = [e for e in events if e.level == "强" and any(k in e.category for k in ("灾祸", "挫折", "波折"))]
    strong_good = [
        e for e in events if e.level == "强" and any(k in e.category for k in ("财运", "事业", "官运", "桃花", "贵人"))
    ]
    ...
    # 中等提示
    mid_bad = [e for e in medium_events if any(k in e.category for k in ("灾祸", "挫折", "波折", "口舌", "是非"))]
    if mid_bad:
