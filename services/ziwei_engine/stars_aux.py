...
def place_aux_stars(info: LunarInfo) -> dict[str, int]:
    ...
    文曲：年支安法 (辰起，顺数)  →  文曲 = (4  + year_branch) % 12 (误，见下)
          实际：辰(4)起子(0)时，戌(10)起丑(1)时...
          文曲 = (4 + year_branch) % 12  ← TODO 寅年起辰，子年起辰-2=寅？
          标准：文曲从辰起子年逆布，辰=4，逆=减
          文曲 = (4 - year_branch + 12) % 12
    天魁：年干安法 (查表)
    天钺：年干安法 (查表)
