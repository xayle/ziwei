...
def _stars_with_hua(p, hua: str) -> list[str]:
    ...
    return [
        s["name"] for s in p.main_stars
        if hua in s.get("transforms", [])
    ]
...
def detect_patterns(palaces: list) -> list[PatternResult]:
    ...
    if "天府" in sanfang_stars and "天相" in sanfang_stars:
        pnames = [
            p.name for p in sanfang
            if "天府" in _main_star_names(p) or "天相" in _main_star_names(p)
        ]
        results.append(PatternResult(
            name="府相朝垣",
            level="吉",
    ...
    if ji_in_life:
        results.append(PatternResult(
            ...
            level="凶",
            description=(
                f"命宫坐化忌（{'/'.join(ji_in_life)}），"
                f"宜防是非口舌与身心损耗，需以德化忌、韬光养晦。"
            ),
            palaces=["命宫"],
            stars=ji_in_life,
            source="《紫微斗数全书》",
    ...
    if cai:
        ...
        if ji_in_cai:
            results.append(PatternResult(
                ...
                level="凶",
                description=(
                    f"财帛宫化忌（{'/'.join(ji_in_cai)}），"
                    f"财务需谨慎管理，防破财、借贷纠纷及意外损耗。"
                ),
                palaces=["财帛宫"],
                stars=ji_in_cai,
                source="《紫微斗数全书》",
    ...
    if guan:
        ...
        if ji_in_guan:
            results.append(PatternResult(
                ...
                level="凶",
                description=(
                    f"官禄宫化忌（{'/'.join(ji_in_guan)}），"
                    f"事业易逢阻碍或压力，宜低调稳健，防职场风波。"
                ),
                palaces=["官禄宫"],
                stars=ji_in_guan,
                source="《紫微斗数全书》",
    ...
    if sun_p and moon_p and sun_p is not moon_p:
        ...
        if ("化禄" in sun_hua or "化权" in sun_hua) and \
           ("化禄" in moon_hua or "化科" in moon_hua):
            results.append(PatternResult(
                name="日月并明",
                level="大吉",
    ...
    if wenhua_stars:
        results.append(PatternResult(
            ...
            palaces=[p.name for p in sizheng if any(
                s["name"] in ("文曲", "文昌") and "化科" in s.get("transforms", [])
                for s in p.main_stars
            )],
            stars=wenhua_stars,
            source="常见论法",
    ...
    _GOOD_STARS = {"紫微","天机","太阳","武曲","天同","廉贞","天府","太阴",
                   "贪狼","巨门","天相","天梁","七杀","破军"}
