...
def _calculate_v1(
    ...
    *,
    ...
    wealth = WealthModel(
        ...
        industry_tags=[
            {"wood": "木", "fire": "火", "earth": "土", "metal": "金", "water": "水"}.get(e, e)
            for e in (yongshen.favor or [])
        ],
    ...
    dayun_model = DaYunModel.model_validate(
        ...
        if hasattr(dayun_model_raw, "model_dump")
    ...
    for item in dayun_model.items:
        ...
        if _s_el in _avd:
            ...
            item.health_hint = "用神旺盛，精力较充沛，是调理旧疾好时机。" + (
...
def _build_month_ganzhis(year_stem: str) -> list[str] | None:
    ...
    return [_STEMS[(start_idx + i) % 10] + _MONTH_BRANCHES[i] for i in range(12)]
...
def _get_current_dayun_stem(
    ...
    _td = date.today()
    current_age = _td.year - birth_year - ((_td.month, _td.day) < (birth_month, birth_day))
...
def _enrich_v2_analysis(
    ...
    try:
        ...
        palace_items = [
            PalaceItemModel(
                ...
                dizhi=br,
                tiangan="",
                strength="旺" if br in (ds_br, ys_br) else "相",
                shishen="",
                note="",
            )
    ...
    def _run_wealth() -> object:
        return compute_wealth(
            yongshen_favor=_favor_snap,
            yongshen_avoid=_avoid_snap,
            wuxing_scores=wx_scores,
            shishen_scores=shishen_scores,
            strength_score=strength_score,
            dayun_list=_dayun_snap,
            day_branch=ds_br,
        )
    ...
    try:
        ...
        if hasattr(verify_response, "dayun") and verify_response.dayun:
            ...
            for _i, _it in enumerate(_nar_items):
                ...
                _end_age = (
                    int((_nar_items[_i + 1].start_age or 0) - 1)
                    if _i + 1 < len(_nar_items)
                    else int((_it.start_age or 0) + 9)
                )
    ...
    try:
        ...
        _rp_branches = (
            ...
            if verify_response.pillars_primary
            else ("", "", "", "")
        )
        ...
        try:
            verify_response.tiangan_clashes = _get_stem_clashes(ys_st, ms_st, ds_st, hs_st, scope="day_related")
    ...
    try:
        ...
        _peak_summary = (
            "、".join(_arc.peak_periods[:2])
            if _arc.peak_periods and "暂无" not in _arc.peak_periods[0]
            else "需参考完整大运排盘"
        )
        _arc_interp = (
            f"{_arc.summary}。"
        ...
        _cautions_short = [
            c.split("（")[0].split("大运")[0].strip() + "大运" for c in _arc.caution_periods[:2] if "暂无" not in c
        ]
        if _cautions_short:
    ...
    try:
        ...
        for yr in range(current_year - 2, current_year + 3):
            ...
            # 判断用神/忌神以生成积极/保守提示
            _is_favor_yr = ystem in {s for s, wx in _TG_STEM_WX.items() if wx in (favor or [])}
            _trend_hint = "用神当令，整体运势向好，宜积极进取" if _is_favor_yr else "宜稳健守成，审时度势"

            _interp = (
                f"{yr}年（{ystem}{ybranch}" + (f"，{_tg}" if _tg else "") + f"）：{_trend_hint}。"
            ...
            _year_optimal = (
                f"{'主动出击' if _is_favor_yr else '稳健守成'}：{_action_text[:35]}" if _action_text else _trend_hint
            )
            if taisui_list:
            ...
            _notable_months = [i + 1 for i, br in enumerate(_MONTH_BR_12) if _BRANCH_WX_M.get(br) in (favor or [])][:3]
    ...
    try:
        ...
        age_now = _today.year - dt.year - ((_today.month, _today.day) < (dt.month, dt.day)) if dt else 0
        ...
        domains = _clf(
            year=current_year,
            year_stem=liunian_stem,
            year_branch=liunian_branch,
            day_stem=ds_st,
            day_branch=ds_br,
            shishen_scores=shishen_scores,
            yongshen_favor=favor,
        ...
        top3 = [f"【{k}】{v}" for k, v in list(domains.items())[:3]]
    ...
    try:
        ...
        _age_yf = _today_yf.year - dt.year - ((_today_yf.month, _today_yf.day) < (dt.month, dt.day)) if dt else 0
