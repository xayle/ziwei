...
# 风险评分 → 等级
_RISK_MAP = {0: "none", 1: "low", 2: "low", 3: "medium", 4: "medium", 5: "medium_high", 6: "medium_high"}
...
def _compute_notable_months(yongshen_favor: list[str]) -> list[int]:
    return [i + 1 for i, br in enumerate(_MONTH_BR_12) if _BRANCH_WX.get(br) in yongshen_favor][:4]
...
def _sig(key: str, label: str, layer: str, severity: str, rule_id: str | None = None) -> EventSignal:
    return EventSignal(
        signal_key=key,
        label=label,
...
def _main_judgment(event_type: str, risk: str, opp: str, year: int, subtypes: list[str]) -> str:
    """生成一句话结论"""
    RISK_DESC = {
        "none": "整体平稳，无明显风险",
        "low": "有轻微波动，总体可控",
...
def _signals_marriage(
    ...
    # ── natal_base ──────────────────────────────────────────────────────────
    if dm_tier in ("偏弱", "极弱"):
        risk_sigs.append(_sig("natal_weak_dm", "日主偏弱受外界影响更大", "natal_base", "tertiary"))

    # ── dayun_trigger ──────────────────────────────────────────────────────
    if dayun_item:
        dy_tg = getattr(dayun_item, "ten_god", "")
        if dy_tg in ("正官", "七杀") and gender == "female":
            risk_sigs.append(_sig("dayun_guansha_female", f"大运{dy_tg}（女命感情压力）", "dayun_trigger", "secondary"))
        elif dy_tg in ("偏财",) and gender == "male":
            opp_sigs.append(_sig("dayun_piancai_male", "大运偏财（男命异性缘旺）", "dayun_trigger", "secondary"))
        elif dy_tg in ("劫财", "比肩"):
            risk_sigs.append(_sig("dayun_bijie_marriage", f"大运{dy_tg}（第三方风险）", "dayun_trigger", "secondary"))
        elif dy_tg in ("食神", "伤官") and gender == "female":
            risk_sigs.append(_sig("dayun_shishang_female", f"大运{dy_tg}女命感情变数", "dayun_trigger", "tertiary"))
    ...
    if clash_val and "冲" in clash_val:
        risk_sigs.append(
            _sig(
                ...
                "liunian_trigger",
                "primary",
                rule_id="marriage_day_branch_clash",
            )
        )  # 本命年（值太岁）——感情流年不稳
    if clash_val and "值太岁" in clash_val:
        risk_sigs.append(
            _sig("ben_ming_nian_marriage", "本命年（值太岁）感情感情容易工于内心", "liunian_trigger", "secondary")
        )
    # 桃花
    ...
    if ten_god == "正官" and gender == "female":
        ...
        risk_sigs.append(
            _sig("guansha_male_stress", f"流年{ten_god}（职场压力波及感情）", "liunian_trigger", "tertiary")
        )
...
def _signals_wealth(
    ...
    dayun_opp = False
    if dy_tg == "劫财":
        risk_sigs.append(_sig("dayun_jiucai", "大运劫财（财运长期不稳）", "dayun_trigger", "secondary"))
        dayun_risk = True
    elif dy_tg in ("偏财", "食神") and dy_wx in yongshen_favor:
        opp_sigs.append(
            _sig(
                "dayun_piancai_wealth",
                f"大运{dy_tg}（财运活跃期）",
                "dayun_trigger",
                "secondary",
        ...
        opp_sigs.append(_sig("dayun_zhengcai", "大运正财（收入稳步积累）", "dayun_trigger", "secondary"))
        dayun_opp = True
    ...
    if ten_god == "劫财":
        risk_sigs.append(
            _sig(
                "liunian_jiucai", "流年劫财（破财合伙风险）", "liunian_trigger", "primary", rule_id="wealth_jiucai_loss"
            )
        )
        ...
        risk_sigs.append(_sig("liunian_piancai_risk", "流年偏财忌神（财来财去）", "liunian_trigger", "tertiary"))
    elif ten_god == "正财":
        opp_sigs.append(
            _sig(
                "liunian_zhengcai",
...
def _signals_property(
    ...
    if liunian_br == yima:
        opp_sigs.append(
            _sig(
                "yima_triggered", "流年临驿马（搬家动象）", "liunian_trigger", "secondary", rule_id="property_yima_move"
            )
        )
    if ten_god in ("正官", "七杀"):
        risk_sigs.append(
            _sig(
                "guansha_property_dispute",
                "流年官杀（房产事宜有阻力）",
                "liunian_trigger",
                "tertiary",
...
def _signals_career(
    ...
    # ── natal_base ──────────────────────────────────────────────────────────
    if dm_tier in ("偏旺", "极旺"):
        opp_sigs.append(_sig("natal_strong_dm", "日主偏旺（事业主动性强）", "natal_base", "tertiary"))

    # ── dayun_trigger ──────────────────────────────────────────────────────
    dy_tg = getattr(dayun_item, "ten_god", "") if dayun_item else ""
    if dy_tg in ("正官", "七杀"):
        opp_sigs.append(_sig("dayun_guansha_career", f"大运{dy_tg}（职场地位受重视）", "dayun_trigger", "secondary"))
    elif dy_tg == "正印":
        opp_sigs.append(
            _sig(
                "dayun_zhengyin_career",
                "大运正印（贵人持续扶助）",
                "dayun_trigger",
                "secondary",
...
def _signals_health(
    ...
    if dominant_ratio > 0.45:
        risk_sigs.append(
            _sig(
                f"dominant_{dominant_el}",
                f"命局{dominant_el}偏旺（对应脏腑长期承压）",
                "natal_base",
                "secondary",
    ...
    if dy_wx and dy_wx in yongshen_avoid:
        risk_sigs.append(
            _sig("dayun_avoid_element", f"大运{dy_wx}五行为忌（体质长期承压）", "dayun_trigger", "secondary")
        )
    elif dy_wx and _OVERCOME.get(dy_wx) == day_wx:
        risk_sigs.append(_sig("dayun_ke_rizhu", f"大运{dy_wx}克日主（精力长期消耗）", "dayun_trigger", "secondary"))
    ...
    if clash_val and "值太岁" in clash_val:
        risk_sigs.append(
            _sig("ben_ming_nian_health", "本命年（值太岁），传统命理认为需防健康不高", "liunian_trigger", "secondary")
        )
    ...
    if liunian_wx == "wood":
        ...
        risk_sigs.append(
            _sig("metal_lung", "流年金旺（肺呼吸道注意）", "liunian_trigger", "tertiary", rule_id="health_metal_lung")
        )
...
def _compute_levels(
    ...
    for s in risk_sigs:
        if s.layer not in risk_layers:
            ...
            risk_score += {"natal_base": 1, "dayun_trigger": 2, "liunian_trigger": 2, "month_trigger": 1}[s.layer]
        risk_score += sev_score.get(s.severity, 0)

    for s in opp_sigs:
        if s.layer not in opp_layers:
            opp_layers.add(s.layer)
            opp_score += {"natal_base": 1, "dayun_trigger": 2, "liunian_trigger": 2, "month_trigger": 1}.get(s.layer, 0)
...
def _infer_subtypes(event_type: str, risk_sigs: list[EventSignal], opp_sigs: list[EventSignal]) -> list[str]:
...
def analyze_multi_year_trend(
    ...
    for year in years:
        try:
            ...
            best_risk = max(events.values(), key=lambda e: RISK_ORD.get(e.risk_level, 0)) if events else None
            best_opp = max(events.values(), key=lambda e: OPP_ORD.get(e.opportunity_level, 0)) if events else None
            ...
            max_risk = (
                max((e.risk_level for e in events.values()), key=lambda r: RISK_ORD.get(r, 0)) if events else "none"
            )
            max_opp = (
                max((e.opportunity_level for e in events.values()), key=lambda o: OPP_ORD.get(o, 0))
                if events
                else "none"
            )
