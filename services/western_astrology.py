...
def _d(r: float) -> float:  return r * 180.0 / math.pi
def _n(d: float) -> float:  return d % 360.0          # 归一化到 [0, 360)
...
def _helio_lon_r(
    L0: float, L1: float, a: float, e: float,
    w: float, O: float, i: float, T: float
) -> tuple[float, float]:
    """
    行星日心黄经（度）和日心距（AU）。
...
def jd_to_datetime(jd: float) -> datetime:
    ...
    return datetime(2000, 1, 1, 12, 0, 0, tzinfo=timezone.utc) + timedelta(days=delta_days)
...
def solar_return_chart(
    ...
    # 从该年的 1 月 1 日开始估算
    dt_start = datetime(sr_year, 1, 1, 0, 0, 0, tzinfo=timezone.utc)
