...
if ($Detach) {
    ...
    if (-not $tempDir) { $tempDir = Join-Path $env:LOCALAPPDATA "Temp" }
    $logOut = Join-Path $tempDir "bazi_dev_${Port}_out.log"
    $logErr = Join-Path $tempDir "bazi_dev_${Port}_err.log"
