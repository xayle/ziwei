...
def handle_exceptions(
    ...
    final_status_code = (
        status_code if status_code is not None else (default_status_code or status.HTTP_500_INTERNAL_SERVER_ERROR)
    )
    ...
    def decorator(func: Callable) -> Callable:
        ...
        async def async_wrapper(*args, **kwargs) -> Any:
            try:
                ...
                logger.error(
                    f"Error in {func.__name__}: {exc!s}",
                    exc_info=True,
                )
                raise AppException(
...
def safe_execute(
    ...
    **kwargs,
    ...
    try:
        ...
        logger.error(
            f"Error executing {func.__name__}: {exc!s}",
            exc_info=True,
        )
        raise AppException(
            code=error_code,
